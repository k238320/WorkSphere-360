...
import { createResource, getResourceByID, updateResource } from 'services/resource';
import { getDepartmentCategory } from 'services/categoryService';
import { createProjectIndustry, getProjectIndustryByID, updateProjectIndustry } from 'services/industryService';
import { createProjectCategoryStatus, getProjectCategoryStatusByID, updatesProjectCategoryStatus } from 'services/categoryStatus';

export function CategoryStatusManageComponent(props: any) {
    ...
    const getCategoryStatusDataByID = async () => {
        dispatch(spinLoaderShow(true));
        getProjectCategoryStatusByID(uuid)
            .then((res: any) => {
                setApiData(res);
    ...
    const createonSubmit = (data: any) => {
        dispatch(spinLoaderShow(true));
        createProjectCategoryStatus(data)
            .then((res: any) => {
    ...
    const updateonSubmit = (data: any) => {
        dispatch(spinLoaderShow(true));
        updatesProjectCategoryStatus(data)
            .then((res: any) => {
    ...
    const onSubmit = (data: any) => {
        if (apiData) {
            ...
            createonSubmit(data);
            navigate('/category_status/listing');
        }
    };
    return (
        <>
